...
def get_grasp(seg,depth,vis):
    ...
    # MODIFY ORIENTATION HERE 19-05-2022
    ang = rect[2]+90
    print('Angle: ',rect[2]+90)
    ...
    rot = [[math.cos(ang*math.pi/180), -math.sin(ang*math.pi/180), 0], [math.sin(ang*math.pi/180), math.cos(ang*math.pi/180), 0],[0,0,1]]
    #print(rot)
    qw = math.sqrt(1+rot[0][0]+rot[1][1]+rot[2][2])/2
    qx = (rot[2][1]-rot[1][2])/(4*qw)
    qy = (rot[0][2]-rot[2][0])/(4*qw)
    qz = (rot[1][0]-rot[0][1])/(4*qw)
    den = qw**2 + qx**2 + qy**2 + qz**2
    mag = math.sqrt(den)
    ...
    gp_rep_str = str(round(pos[0][0],3))+','+str(round(pos[1][0],3))+','+str(round(pos[2][0],3))+','+str(round(quat[0],3))+','+str(round(quat[1],3))+','+str(round(quat[2],3))+','+str(round(quat[3],3))+','+'1,-1,0,4,'
    print("GRASP REP: ",gp_rep_str)
    if w_cam<=50:
        g_able = True
...
def disp_data_received(quaternion,translation):
    port = 60064
    quaternion = [round(x,5) for x in quaternion]
    quaternion = [str(x) for x in quaternion]
